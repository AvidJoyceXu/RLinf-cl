...
_NAME = {
    "name": {
        "type": "string",
        "description": "sim object name, as reported by observe()",
    }
}
_SYS = {
    "system_name": {"type": "string", "description": "particle/substance system name"}
}
...
def scan_schemas(*, memory: bool = False) -> list[dict]:
    """Oracle discovery tools for the text-only scan branches."""
    tools = [
        {
            "name": "scan_next",
            "description": (
                "Advance one step in the deterministic scene-scan cursor and return "
                "the new category-labelled detection view. This is an oracle "
                "viewpoint controller: it accepts no target or direction."
            ),
            "parameters": _obj({}, []),
        }
    ]
        ...
        tools.append(
            {
                "name": "scan_at",
                "description": (
                    "Revisit a scene-stable lN landmark and directly return its "
                    "detection view. Use only a landmark id observed in this scene."
                ),
                "parameters": _obj(
                    {
                        "landmark": {
                            "type": "string",
                            "description": "scene-stable lN landmark id",
                        }
                    },
                    ["landmark"],
                ),
...
PRIMITIVE_CAMERA_OBS_MODES = ("fov", "fov_distract", "detect", "detect_scope")
EPISODE_SCAN_OBS_MODES = ("detect_scope_scan",)
MEMORY_SCAN_OBS_MODES = ("detect_scope_memory",)
SCAN_OBS_MODES = EPISODE_SCAN_OBS_MODES + MEMORY_SCAN_OBS_MODES
CAMERA_OBS_MODES = PRIMITIVE_CAMERA_OBS_MODES + SCAN_OBS_MODES
DETECT_OBS_MODES = ("detect", "detect_scope") + SCAN_OBS_MODES
SELECTION_MODES = ("handle", "bbox")


def _detect_selection_schema(tools: list[dict], selection_mode: str) -> None:
    """Rewrite object parameters to match the policy-visible detect channel.

    This mutates only the newly-created schema list.  Non-detect modes retain the
    historical name-addressed schema byte-for-byte.
    """
    if selection_mode not in SELECTION_MODES:
        raise ValueError(
            f"selection_mode must be one of {SELECTION_MODES}, got {selection_mode!r}"
        )
    selector = (
        "view-local dN handle from the immediately preceding observation"
        if selection_mode == "handle"
        else "bbox or point copied from the immediately preceding observation, "
        "encoded as 'x0,y0,x1,y1' or 'x,y'"
    )
    by_name = {tool["name"]: tool for tool in tools}
    by_name["observe"]["description"] = (
        "Return the current camera view as category-labelled detections and the "
        "proprioceptive held-object reference. Every camera or successful visual "
        "world change invalidates earlier detections."
    )
    for tool in tools:
        ...
        for target_key in ("surface", "container"):
    ...
    # The movable disappears from detections after grasp.  It is proprioceptive,
    # not a special bbox/handle that survives a world revision.
    by_name["release"]["parameters"]["properties"]["name"] = {
        "type": "string",
        "description": "reserved literal 'held'; may also be omitted",
    }
    for name in ("place_on", "place_inside"):
        by_name[name]["parameters"]["properties"]["name"] = {
            "type": "string",
            "description": "reserved literal 'held' for the currently held object",
            "enum": ["held"],
        }
...
def all_schemas(
    ...
    """Every tool the policy may call under ``obs_mode`` and selection channel."""
    tools = tool_schemas(minimal)
    if obs_mode in DETECT_OBS_MODES:
        _detect_selection_schema(tools, selection_mode)
    elif selection_mode not in SELECTION_MODES:
        raise ValueError(
            f"selection_mode must be one of {SELECTION_MODES}, got {selection_mode!r}"
        )
    if obs_mode in SCAN_OBS_MODES:
        return tools + scan_schemas(memory=obs_mode in MEMORY_SCAN_OBS_MODES)
    return tools + (camera_schemas() if obs_mode in PRIMITIVE_CAMERA_OBS_MODES else [])
...
def build_prompt_messages(
    ...
    if goal_nl is not None:
        ...
        body = (
            "Complete the activity so that all of these goal conditions hold:\n"
            f"{goal_block}"
        )
    ...
    if prompt_mode == "minimal":
        ...
        obs_line += (
            " -- you see only what is in this room and not shut inside a "
        ...
        SWEEP = (
            "Use turn_left / turn_right to sweep, move_ahead to approach, "
            "look_down for low surfaces, then observe again."
        )
        # The distractor axis.
        MIXED = (
            "Scene furniture and task objects are both reported and are not "
            "distinguished."
        )
        if obs_mode in DETECT_OBS_MODES:
            if selection_mode not in SELECTION_MODES:
                raise ValueError(
                    f"selection_mode must be one of {SELECTION_MODES}, got "
                    f"{selection_mode!r}"
                )
            if selection_mode == "handle":
                channel = (
                    "a view-local handle (d1, d2, ...), a category and a score. "
                    "Refer to an object only by its handle, e.g. grasp(name='d3'). "
                )
            else:
                channel = (
                    "a bounding box, a category and a score. Refer to an object only "
                    "by a copied box 'x0,y0,x1,y1' or point 'x,y'. "
                )
            obs_line += (
                " -- observe() returns detections in the camera's view: " + channel
            )
            if obs_mode == "detect":
                obs_line += MIXED + " "
            obs_line += (
                "A detection reference is valid only until the camera or visual "
                "world state changes; obtain a current view before selecting. "
            )
            if obs_mode in SCAN_OBS_MODES:
                obs_line += (
                    "Call scan_next() to advance a public deterministic scan cursor; "
                    "it takes no target and directly returns the next view. Each "
                    "detection has a stable sN track across views; remember that "
                    "track and use its current dN handle when acting. Bind repeated "
                    "same-category goal roles in first-seen track order."
                )
                if obs_mode in MEMORY_SCAN_OBS_MODES:
                    obs_line += (
                        " The sN object track is activity-local and resets between "
                        "tasks. The scene_id and lN landmarks are session-stable; "
                        "remember useful lN locations and call scan_at(landmark='lN') "
                        "to revisit them in a later activity in the same scene."
                    )
            else:
                obs_line += SWEEP
        else:
            ...
            if obs_mode == "fov_distract":
                obs_line += MIXED + " "
            obs_line += SWEEP + " You can only go_to an object you have already seen."
    user = f"Activity: {activity.replace('_', ' ')}\n{obs_line}\n{body}"
    return [
        {
            "role": "system",
            "content": (
    ...
    tool_steps.append(
        {
            ...
            "result_text": _result_text(
                {"ok": bool(traj.success), "success": bool(traj.success)}
            ),
        }
    )
    return {
        "activity": traj.activity,
        "task": getattr(traj, "task_description", "")
        or activity_to_task(traj.activity),
...
if __name__ == "__main__":
    ...
    fake = SimpleNamespace(
        activity="carrying_in_groceries",
        task_description="",
        success=True,
        unsupported=[],
        steps=[
            SimpleNamespace(
                tool="go_to",
                args={"name": "fridge_xyz"},
                ok=True,
                reason="now 0.9m from fridge_xyz",
            ),
            SimpleNamespace(
                tool="open", args={"name": "fridge_xyz"}, ok=True, reason="Open=True"
            ),
            SimpleNamespace(
                tool="grasp",
                args={"name": "bratwurst_1"},
                ok=True,
                reason="holding bratwurst_1",
            ),
            SimpleNamespace(
                tool="place_inside",
                args={"name": "bratwurst_1", "container": "fridge_xyz"},
                ok=True,
                reason="Inside=True",
            ),
            SimpleNamespace(
                tool="close", args={"name": "fridge_xyz"}, ok=True, reason="Open=False"
            ),
        ],
    )
    obs = {
        ...
        "held": None,
        "objects": [
            {"name": "fridge_xyz", "category": "fridge", "states": {"Open": False}}
        ],
    }
    ...
    assert (
        "inside(bratwurst.n.01_1, fridge.n.01_1)"
        in row["prompt_messages"][1]["content"]
    )
