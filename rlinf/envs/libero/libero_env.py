...
class LiberoEnv(gym.Env):
    ...
    def _wrap_obs(self, obs_list):
        ...
        for obs in obs_list:
            # Extract VLA observations (images and states)
        ...
        # Add RL observations if available
        if rl_obs_list:
            # Convert to numpy arrays and stack
            robot_proprio_states = np.stack([r["robot_proprio_state"] for r in rl_obs_list])
            
            # Handle object_to_robot_relations (may be empty)
            relations_list = [r["object_to_robot_relations"] for r in rl_obs_list]
        
            object_to_robot_relations = np.stack(relations_list)
            
            obs["robot_proprio_state"] = robot_proprio_states
            obs["object_to_robot_relations"] = object_to_robot_relations
